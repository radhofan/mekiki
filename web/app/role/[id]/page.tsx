...
import type { JobRole, LeaderboardEntry, QualificationStatus } from '@/lib/types';
...
export default function LeaderboardPage() {
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    setError(null);
    Promise.all([getRole(roleId), getLeaderboard(roleId)])
      .then(([r, lb]) => {
        setRole(r);
        setBoard(lb);
      })
  ...
  const handleBatch = async () => {
    ...
    setBatchMsg(null);
    try {
      const res = await batchEvaluate(roleId);
    ...
    <div className='p-8 max-w-6xl mx-auto'>
      ...
      <div className='flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8'>
        <div>
          ...
          <p className='text-slate-500 text-sm'>
            {role?.department && <span className='mr-2'>{role.department} ·</span>}
        ...
        <div className='flex items-center gap-3 shrink-0'>
          ...
          <Button
            onClick={handleBatch}
            disabled={batching}
            className='bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white font-medium shadow-lg shadow-violet-500/20'
          >
            {batching ? (
      ...
      {batchMsg && (
        <div className='mb-6 p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-sm'>
          {batchMsg} Results will appear shortly — click Refresh.
        </div>
      )}
      ...
      {role && (
        <div className='glass p-4 rounded-xl mb-6 grid grid-cols-1 md:grid-cols-2 gap-4'>
          <div>
            ...
            <p className='text-sm text-slate-300 line-clamp-3'>{role.description}</p>
          </div>
          <div>
            <p className='text-xs text-slate-500 uppercase tracking-wider mb-1'>Required Skills</p>
        ...
        <div className='space-y-2'>
          {board.map((entry) => (
            <button
              ...
              className='w-full glass flex items-center gap-4 p-4 rounded-xl hover:bg-white/[0.04] transition-all duration-150 text-left group'
            >
              {/* Rank */}
              <div className='w-8 shrink-0 text-center'>
                {entry.rank <= 3 ? (
                  ...
                  <span className='text-slate-600 font-mono text-sm'>#{entry.rank}</span>
                )}
              </div>
              ...
              <div className='flex-1 min-w-0'>
                ...
                {entry.email && (
                  <p className='text-xs text-slate-600 truncate'>{entry.email}</p>
                )}
              </div>
              ...
              <div className='w-36 shrink-0 hidden sm:block'>
                <div className='flex items-center justify-between mb-1'>
                  <span className={`text-xs font-bold ${scoreColor(entry.match_score)}`}>
                    {entry.match_score}%
                  </span>
                </div>
                <Progress
            ...
            <>
              ...
              <div className='flex items-center gap-6 py-4 border-b border-white/5'>
                ...
                <div>
                  ...
                  {selected.email && (
                    <p className='text-xs text-slate-500 mt-2'>{selected.email}</p>
                  )}
                </div>
              </div>
              ...
              <div className='mt-4'>
                ...
                <p className='text-sm text-slate-300 leading-relaxed bg-white/[0.03] rounded-lg p-4 border border-white/5'>
                  {selected.ai_justification}
                </p>
              </div>

              {/* Extracted skills */}
              {selected.extracted_skills && selected.extracted_skills.length > 0 && (
                <div className='mt-4'>
                  <p className='text-xs text-slate-400 uppercase tracking-wider mb-2'>
                    Identified Skills
                  </p>
              ...
              <p className='text-xs text-slate-600 mt-6'>
                Evaluated {new Date(selected.evaluated_at).toLocaleString()}
              </p>
            </>
          )}
        </DialogContent>
